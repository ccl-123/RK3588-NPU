...
| 指标 | 数值 | 说明 |
|------|------|------|
| **识别帧率** | 35-38 FPS | 命令行版本（流水线架构） |
| **GUI 帧率** | 30-35 FPS | GUI 版本（含渲染） |
| **识别准确率** | >95% | 良好光照条件下 |
| **识别延迟** | ~35ms | 单人脸 |
| **YOLO检测** | ~26ms | 瓶颈阶段 |
| **FaceNet识别** | ~6ms | 特征提取 |
...
│  PreprocessingThread, RecognitionThread │
├─────────────────────────────────────────┤
│         Core Layer                      │
│  YOLOv8-face, FaceNet, Postprocess      │
├─────────────────────────────────────────┤
│         Hardware Layer                  │
...
```
线程1 [采集+RGA]  ──队列──>  线程2 [YOLO检测]  ──队列──>  线程3 [识别+渲染]
     ~1.4ms                    ~26ms (瓶颈)               ~8.6ms
```
...
1. **流水线架构**: 三线程并行，最大化 NPU 利用率
2. **高性能**: RKNN NPU 加速，35-38 FPS 实时识别
3. **线程安全**: 数据库连接池、递归锁保护
4. **模块化设计**: 六层架构，职责清晰
5. **用户友好**: Qt GUI 界面，操作简单
6. **数据持久化**: SQLite 数据库，支持事务
7. **可扩展性**: 支持多种摄像头、可配置参数
8. **专业日志**: spdlog 集成，便于调试
9. **跨平台**: 理论上支持其他 ARM 平台
